...
public class Functions(ILogger<Functions> logger)
{
  ...
  public async Task SendWeeklyEmails([TimerTrigger("0 0 8 * * 1", RunOnStartup = isDebug)] TimerInfo timer, [BlobInput("config")] BlobContainerClient container)
  {
    ...
    var teams = new TeamsClient(credentials, logger);
    var blobs = await Resilience.ExecuteAsync(async () =>
    {
      var loadedBlobs = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
      await foreach (var blob in container.GetBlobsAsync())
      {
        var download = await container.GetBlobClient(blob.Name).DownloadContentAsync();
        loadedBlobs.Add(blob.Name, download.Value.Content.ToString());
      }
      return loadedBlobs;
    }, logger, "Load blob configuration");
    ...
    var failedSchools = new List<string>();
    ...
    foreach (var schoolCode in schoolCodes) {
      try
      {
        ...
        var mailer = new Mailer(postmarkServerToken, schoolCode, $"{school.Name} <{school.FromEmail}>", school.TeachersByCode[school.ReplyTo].Email, debugEmail, logger);
      ...
      {
        failedSchools.Add(schoolCode);
        logger.LogError(ex, "{School} - Failed.", schoolCode);
      }
    }

    if (failedSchools.Count > 0)
      throw new InvalidOperationException($"Failed to process schools: {string.Join(", ", failedSchools)}");
  }
  ...
  private static IEnumerable<string[]> GetCsv(string text)
  {
    ...
    public static DateOnly Today { get; } = new(2026, 6, 15);
  #else
